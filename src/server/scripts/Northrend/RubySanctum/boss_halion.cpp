...
enum
{
    ...
    SPELL_TWILIGHT_CUTTER                       = 74769, // 68114  77844 74769dont work (( Inflicts 13,875 to 16,125 Shadow damage every second to players touched by the shadow beam
...
//        me->SetDisplayId(10045);
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (m_timer <= uiDiff)
            {
                ...
                /* part taken from scriptdev repository, doesn't work
                Map* pMap = me->GetMap();
                ...
                for(Map::PlayerList::const_iterator i = pList.begin(); i != pList.end(); ++i)
                {
                    if (Player* player = i->getSource())
                    {
                        ...
                        if (player->isAlive())
                        {
                            float AB = pPulsar1->GetDistance2d(pPulsar2)+1;
                            float BC = pPulsar1->GetDistance2d(player)+1;
                            float AC = pPulsar2->GetDistance2d(player)+1;
                            float p = (AB + BC + AC)/2;
                            ...
                            {
                                player->CastSpell(player, SPELL_TWILIGHT_CUTTER, true, 0, 0, pPulsar1->GetGUID());
                            }
                        }
                    }
                }
...
public:
    ...
    struct mob_halion_meteorAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            me->SetRespawnDelay(7*DAY);
            SetCombatMovement(false);
            m_uiMeteorImpactTimer = uint32(0.5*IN_MILLISECONDS);
            m_uiMeteorStrikeTimer = uint32(4.5*IN_MILLISECONDS);
            setStage(0);
            me->SetInCombatWithZone();
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
...
class spell_twilight_cutter : public SpellScriptLoader
{
    public:
        ...
        class spell_twilight_cutter_SpellScript : public SpellScript
        {
            ...
            {
                if (!sSpellStore.LookupEntry(SPELL_TC))
                    return false;
                return true;
            }

            void DamageBetween(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    for(Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
                    {
                        ...
                        {
                            if(pPlayer->IsInBetween(caster, target, 100.0f))
                                caster->CastSpell(pPlayer, SPELL_TWILIGHT_CUTTER, false);
                        }
                    }
                }
            }
            ...
            void Register()
            {
                OnEffect += SpellEffectFn(spell_twilight_cutter_SpellScript::DamageBetween, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };
